 ...
 */
function _default(realUid, cookie, ip) {
  console.log(ip);

  if (!cookie) {
    cookie = _spider["default"].find('cookie').exec();
  }

  return new Promise(function (resolve, reject) {
    try {
      request({
        url: "http://www.jiayuan.com/".concat(realUid),
        method: 'GET',
        headers: {
        ...
        try {
          ...
          var all = {};
          var infoLis = $('.member_info_list li');
          ...
          var zoyqLsit = jsArrs.eq(2).find('.js_list li');
          var ZQYQ = {};
          ...
          all['择偶要求'] = ZQYQ;
          var shxg = jsArrs.eq(3).find('.js_list li');
          var SHFS = {};
          SHFS['嗜好习惯'] = {};
          ...
          all['生活方式'] = SHFS;
          var jjslArr = jsArrs.eq(4).find('.js_list li');
          var JJSL = {};
          ...
          all['经济实力'] = JJSL;
          var gz = jsArrs.eq(5).find('.js_list').eq(0).find('li');
          var xx = jsArrs.eq(5).find('.js_list').eq(1).find('li');
          var GZXX = {};
          GZXX['工作'] = {};
          ...
          all['工作学习'] = GZXX;
          var gyzj = jsArrs.eq(6).find('.js_list').eq(0).find('li');
          var gyjt = jsArrs.eq(6).find('.js_list').eq(1).find('li');
          var HYGN = {};
          HYGN['关于自己'] = {};
          ...
          all['照片'] = {
            'big': bigImgArr,
            'small': smallImgArr // fs.writeFileSync('detail1.json', JSON.stringify(all)) //TODO!
